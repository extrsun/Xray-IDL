...
;
;*CALLING SEQUENCE:
;       RSGETASPQU, inputs, obinum, SCTIME, QUALFLAG, STARFLAG, NOMASP,
...
;    mode 1) Interactive
;        IDL>rsgetaspqu,1,obinum,sctime,qualflag,starflag, ...
;             ?obseq=rp123456
;	      ?dir=mydirectory
;	      ?trim=1
;	      ?chatter=0
...
;        IDL>rsgetaspqu,list,obinum,sctime,qualflag,starflag, ...
;  
...
;	     chatter=0
;            *exit
;        IDL>rsgetaspqu,'myinput.dat',obinum,sctime,qualflag,starflag, ...
;
;*RESTRICTIONS:
...
;-
;-------------------------------------------------------------------------------
pro rsgetaspqu,inputs,obinum,sctime,qualflag,starflag,nomasp,obinfo=obinfo,oparms=oparms
;
npar = n_params(0)
if (npar eq 0) then begin
  print,' RSGETASPQU, inputs, obinum (0 for all), SCTIME, QUALFLAG, STARFLAG,'
  print,'             NOMASP, OBINFO=OBINFO, OPARMS=OPARMS'
  print,'   Uses inputs OBSEQ, DIR, EXTYP, INSTR, TRIM, and CHATTER from RSGET.DEF'
  return
endif
;
dfile = 'rsget'
...
if (extuc ne 'ASP') then begin                  ;don't know HRI extension yet
  print,extyp,' is not a valid extension for an aspect data file.'
  print,' Valid options are ASP (PSPC). Returning.'
  retall 
endif
;
;ext = '_'+extyp+'_*.hhh'
;match_files,dir,obseq,ext,names,nlist,obinum       ;look for the right files
ext = '.'+extyp+';*'
...
;
  if (npar ge 6) then begin
    ...
    if (!debug ge 2) then stop,'Stopping after getting nominal aspect'
  endif
;
endfor
;

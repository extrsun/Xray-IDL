;========================================
pro cr_gal_mult,lld,bbd,noo,ttlo,tthi,ttn,gglo,gghi,ggn,crov,gamma=gammao $
 ,nha=nhao,sel=sel,nch=nch,ttv=ttv,ggv=ggv,blow=blow,bhigh=bhigh,fname=fname
;+
; main program for calculating the surface intensity of the corona, 
;	accounting for the absorption.
...
ggv=gglo+(gghi-gglo)/(ggn-1.)*findgen(ggn)
crov=fltarr(nbin,ttn,ggn)
for kg=0,ggn-1 do begin
for k=0,ttn-1 do begin
	cr_gal,lld,bbd,noo,ttv(k),cro,gamma=ggv(kg),nha=nhao,sel=sel,nch=nch, $
...
;	cro(k)=simpson(func,0.,rrlimit,tol=tol)
	...
	cro(k)=total(cro_sb(rr,indlo=indlo(k),indhi=indhi(k)))*step
endfor
return
end
;================================================
...
rz2=zz^2+rrc^2
ph2=-3.0*vv/(0.70+sqrt((zz*1.64)^2+rrc^2))

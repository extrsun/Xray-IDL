...
sh_opac,nhv*0.01>0.,op1,bchn+1,opf=opfile,/nolog,/noim
op=[op,op1]
dvsz=size(dv)
ratioth=rhi
if n_elements(selo) eq 0 then selo=lindgen(dvsz)
nsel=n_elements(selo)
seln=selo
seln2=[seln,dvsz(1)+seln]
wws=1.
func='func_powell'
...
;	wws=1./sqrt(tcr > 1.e-10)
	wws=1./(tcr > 1.e-10)
	ratio=reform((dvs-tcr)*wws,nsel,2)
...
; 	tv,bscale(imm,0.,5),k
	...
	ss=where(abs(ratio(*,0)) le ratioth and $
		abs(ratio(*,1)) le ratioth ,nsel)
	seln=seln(ss)
	ss2=[ss,nsel+ss]
	seln2=[seln,dvsz(1)+seln]
	wws=wws(ss2)^2

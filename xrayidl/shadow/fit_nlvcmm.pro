pro fit_nlvc,nhvc,countvc,backvc,timevc,xflux,band=band $
,opfile=opfile,mflux=mflux,chimin=chimin,pmin=pmin,pmax=pmax,pfix=pfix,gnhvc=gnhvc,noconf=noconf,plotoff=plotoff,xflux_ini=xflux_ini,gopfile=gopfile,ww=ww
;++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
;+
;**NAME:
; fit_nlvc
...
;**OPTIONAL INPUTS:
; 
; bnlo, bnhi - the lower and upper limits of the bands to be included (def:0)
;	The definition of the bands are 0,1,2,3,4,5,or 6 = 
;	'1_2','4_5','6_7','1_1','2_2','3_3','4_4', respectively.
...
print,'CALLING SEQUENCE --- fit_nlvc,nhvc,countvc,backvc,timevc,xflux'
print,',bnlo=bnlo,bnhi=bnhi,opfile=opfile,mflux=mflux,chimin=chimin'
print,',pmin=pmin,pmax=pmax,pfix=pfix,gnhvc=gnhvc,gopfile=gopfile'
return
endif
timevco=timevc
...
if n_elements(pmin) eq 0 then pmin=[replicate(1.e-10,ncomp*n_band),0.1]
if n_elements(pmax) eq 0 then pmax=[replicate(1.,ncomp*n_band),5]
;print,'pmin=',pmin
;print,'pmax=',pmax
...
for nb=0,n_band-1 do begin
	...
	if n_elements(gnhvc) ne 0 then begin
		...
		sh_opac,gnhvc*0.01,gopacity,band(nb),quad=quad,opfile=gopfile
		gop(*,nb)=gopacity
	endif
endfor
x=lindgen(ncol*n_band) ;  only the dimension info is to be used
xflux=dblarr(ncomp*n_band+1)
...
if n_elements(xflux_ini) eq 0 then begin
 for nb=0,n_band-1 do begin
	xflux(0+nb*ncomp)=mf(nb)*0.6 ;these are arbitrarily chosen values
	xflux(1+nb*ncomp)=mf(nb)*0.4
 endfor
 xflux(ncomp*n_band)=1. ;the last parameter is assumed to contain the 
			; normalization 
...
vbackvc=reform(backvc,ncol*n_band) ;one dimension data points for
			;multi-nb data into curvefit_mm
...
;		ploterr,nhvc,flux,eflux
		ploterr,nhvc,flux(*,0),eflux(*,0),psym=5 $
			,yrange=[min(flux-eflux),max(flux+eflux)]
		oplot,nhvc,mflux(*,0)
	endif
if n_band gt 1 then begin
	if keyword_set(plotoff) eq 0 then begin
	  for kk=1,n_band-1 do begin
		oploterr,nhvc,flux(*,kk),eflux(*,kk),psym=5-kk
		oplot,nhvc,mflux(*,kk)
	  endfor
	endif
endif
...
for band=0,n_band-1 do begin
 ...
 mpder(band*np:(band+1)*np-1,band*ncomp+1)=timevc(*,band) $
		*exp(para(npara-1)*op(*,band)+gop(*,band))
 mpder(band*np:(band+1)*np-1,npara-1)=timevc(*,band)*para(1+ncomp*band) $
		*exp(para(npara-1)*op(*,band)+gop(*,band))*op(*,band)
endfor
...
for band=0,n_band-1 do begin
  mcount(*,band)=timevc(*,band)*(para(ncomp*band) $
	+para(1+ncomp*band)*exp(para(npara-1)*op(*,band)+gop(*,band)))
endfor

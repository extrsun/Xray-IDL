...
common func_para2,choice,dvs,mas,ttv,ggv,gg,wws,ops,para,tcr,masn
nttv=n_elements(ttv)
if tt gt ttv(nttv-1) or (tt lt ttv(0)) then chi=1.e20 $
		; effective set the boundaries of the model parameters
nggv=n_elements(ggv)
if gg gt ggv(nggv-1) or (gg lt ggv(0)) then chi=1.e20 $
		; effective set the boundaries of the model parameters
	else begin
;	get_masn,mas,ttv,tt,masn
	get_masn2,mas,ttv,tt,ggv,gg,masn
...
tabinv,ttv,tt,ttlo
ttlo=fix(ttlo) < (n_elements(ttv)-2)
tthi=ttlo+1
sz=size(mas)
if sz(0) eq 2 then $
 masn=((ttv(tthi)-tt)/(ttv(tthi)-ttv(ttlo)))*mas(*,ttlo)+ $
	((tt-ttv(ttlo))/(ttv(tthi)-ttv(ttlo)))*mas(*,tthi) $
else begin
 masn=fltarr(sz(1),sz(3))
 for k=0,sz(3)-1 do masn(*,k)=((ttv(tthi)-tt)/(ttv(tthi)-ttv(ttlo))) $
	*mas(*,ttlo,k)+((tt-ttv(ttlo))/(ttv(tthi)-ttv(ttlo)))*mas(*,tthi,k)
endfor
return
end
;===================================================================
pro get_masn2,mas,ttv,tt,ggv,gg,masn

tabinv,ttv,tt,ttlo
ttlo=fix(ttlo) < (n_elements(ttv)-2)
tthi=ttlo+1
tabinv,ggv,gg,gglo
gglo=fix(gglo) < (n_elements(ggv)-2)
gghi=gglo+1
masglo=((ttv(tthi)-tt)/(ttv(tthi)-ttv(ttlo)))*mas(*,ttlo,glo)+ $
((tt-ttv(ttlo))/(ttv(tthi)-ttv(ttlo)))*mas(*,tthi,glo)
masghi=((ttv(tthi)-tt)/(ttv(tthi)-ttv(ttlo)))*mas(*,ttlo,ghi)+ $
((tt-ttv(ttlo))/(ttv(tthi)-ttv(ttlo)))*mas(*,tthi,ghi)
masn=((ggv(gghi)-gg)/(ggv(gghi)-ggv(gglo)))*masglo+ $
((gg-ggv(gglo))/(ggv(gghi)-ggv(gglo)))*masghi
return
end
;========================================

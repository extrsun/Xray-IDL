pro cast_psf_main,filelist,mhdr,mmap,fdir=fdir,ftail=ftail,mtype=mtype $
,offset=offset,nozero=nozero,avgkey=avgkey,probth=probth $
,slow=slow,flow=flow,soufile=soufile,instr=instr,subfac=subfac,psffile=psffile,eband=eband,mtoi=mtoi,nonorm=nonorm,fname=fname,nointerp=nointerp,tanorm=tanorm,perclimit=perclimit,difexp=difexp,inr=inr,outr=outr,inarr=inarr
;+
...
;
;*OPTIONAL Inputs:
; perclimit - PSF EER fraction, if given psf image is calculated
; difexp - if set, psf image is calculated for each individual file
;          (e.g., exposure map size and/or center are different)
...
print,',slow=slow,flow=flow,souf=soufile,instr=instr,subfac=subfac,psffile=psffile,eband=eband,nonorm=nonorm,fname=fname,tanorm=tanorm,perclimit=perclimit,difexp=difexp,inr=inr,outr=outr'
return
endif
excth=n_elements(inr) +n_elements(outr)
if excth ne 0 then begin
    if n_elements(inr) eq 0 then inr=0
    if n_elements(outr) eq 0 then outr=1.e22
endif
if n_elements(mtype) eq 0 then mtype=0
if n_elements(fdir) eq 0 then fdir='../xdata/'
if n_elements(ftail) eq 0 then ftail=''
;-----
if n_elements(eband) eq 0 then eband=1
...
for kk=0,n_elements(nfname)-1 do begin
 if exist(nfname(kk)) then begin
    ...
    endelse 
    if n_elements(perclimit) ne 0 or excth ne 0 then begin 
        if kk eq 0 or keyword_set(difexp) ne 0 then begin 
            ...
            if n_elements(perclimit) ne 0 then $
              psf_params,dd,rs,perclimit=perclimit,psffile=psffile $
                   ,blow=bmin(eband-1),bhigh=bmax(eband-1),spfile=spfile
            if excth ne 0 then exc=where(dd lt inr or dd gt outr,nexc)
        endif 
        if nexc ne 0 then inarr(exc)=0 
        if n_elements(perclimit) ne 0 then begin
            ss=where(inarr gt 0.,nss)
            if nss gt 0 then inarr(ss)=inarr(ss)*rs(ss)^2
        endif 
    endif 

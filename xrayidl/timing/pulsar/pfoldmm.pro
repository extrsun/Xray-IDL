...
;		fac=1.0-(time/tfar)^2/3.0d
		t2n=(time/tfar)^2/3.0d
		ffarn=ffar-fdot*tfar
		if ffarn lt fmax then fmax=ffarn
		if fmax le fmin then stop,'fmax < fmin'
	endif
...
;	pht=mtime*(ff+hfdot+(ffar-ff-fdot*tfar)/3.0d*(time/tfar)^2)
;	pht=mtime*(ff+hfdot+(ffarn-ff)*t2n)
;	aaa=(ffarn-ff)*t2n
;	if min(aaa) le 0. then stop,'aaa < 0'
	if n_elements(ffar) ne 0 then pht=mtime*(ff+hfdot+(ffarn-ff)*t2n) $
	else pht=mtime*(ff+hfdot)
...
if n_elements(ffar) ne 0 then	phase=mtime*(fbest+hfdot+(ffarn-ff)*t2n) else $
...
if keyword_set(plotoff) eq 0 then $

pro ppdot_fold,time,fmin,fmax,minfdot,maxfdot,phd=phd,vfv=vfv,vdv=vdv,ddf=ddf $
 ,fv=fv,dv=dv,phase=phase,fbest=fbest,dmax=dmax,fdotbest=fdotbest,tmin=tmin $
 ,filter=filter,efilter=efilter,ffix=ffix,df=df
...
for k=0,nfdot do begin
	...
	pfold,time,fmin,fmax,fdot(k),phd=phd,fbest=fbest,dmax=dmax $
		,tmin=tmin,fv=fv,dv=dv,/plotoff,ddf=ddf,df=df $
		,filter=filter,efilter=efilter,ffix=ffix  ;,/proc_m
...
pfold,time,fbesto-fint,fbesto+fint,fdotbest,fbest=fbest,dmax=dmax $
	,tmin=tmin,phd=phd*0.5,fv=fv,dv=dv,phase=phase,ddf=ddf $
	,filter=filter,efilter=efilter,ffix=ffix,df=df
stop
return
end
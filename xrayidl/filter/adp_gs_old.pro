...
while ni lt nimax do begin
	...
	if nback ne 0 then imt=imc+back else imt=imc
	;if nback ne 0 then imssigma=(imc+back)/(1.50*binsize)^2 else $
	imssigma=imt/(1.50*binsize)^2  
	if choice eq 2 then begin
		imssigma=imdiv(imssigma,expt) ;to get c/(t*t)
		fton=imdiv(imt,sqrt(imssigma)) 
			;to avoid problems with imc \sim 0.
	endif else fton=imdiv(imc,sqrt(imssigma))

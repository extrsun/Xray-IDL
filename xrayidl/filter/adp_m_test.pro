...
while ni lt nimax and binsize lt gshi do begin
    print,'ni, binsize,nsel = ',ni,binsize,nsel
     ...
     for k=0,nima-1 do begin
         if nback ne 0 then ime(*,*,k)=convolve(expta(*,*,k),wlimg^2) $
           else ime=convolve(expta(*,*,k)*fluxac(*,*,k)+back(*,*,k),wlimg^2)) 
     endfor
     ftona=imdiv(imac,sqrt(ime))
     fton=imdiv(total(imac,3),sqrt(total(ime,3)))

...
; -----------------------------------
; modifications: (1) use a procedure get_detmap for creating a broad band ;!
;	(2) get the modififed defimap to replace 11_19 channel with 
; 	8_19 channel when an observation was taken after the gain change of
;	the PSPC B; (3) include keywords: BANDWS, ZERO, EXPTAIL,and BANDCR. 
; (modifications were done in the program with the mark ;!). WQD, Oct. 12, 1993
;
; BANDWS - THE WIDTH OF A BROAD BAND IN UNITS OF BANDS (1-7)
; 	   SO THAT A BROAD BAND EXPOSURE MAP CAN BE PRODUCED WITH USER SUPPLIED
;	   ENERGY SPECTRUM
...
;imapdir=imapdir,oparms=oparms
PRO MAKE_EMAP_M,INPUTS,EXPMAP,TOTEXP,EMAPHDR,ACTIME=ACTIME,BANDS=BANDS,$ ;!
    IMAPDIR=IMAPDIR,OPARMS=OPARMS,BANDWS=BANDWS,ZERO=ZERO,EXPTAIL=EXPTAIL $
    ,BANDCR=BANDCR,poehdr=poehdr,obdate=obdate
;
;
...
;
if ( (obseq eq '') and (proc ne 'MPE') ) then begin   ;obseq must be defined
...
;
if n_elements(poehdr) eq 0 then begin
ext = '.fits;*'
if (proc eq 'MPE') then ext = '_events.tfits'
if (proc eq 'MPE') then ext = '_bas.fits'
...
if n_elements(obdate) eq 0. then $
rsobsdate,poehdr,date,proc=proc else $        ;get the observation date
	date=obdate
...
;
IF (N_ELEMENTS(IMAPDIR) EQ 0) THEN IMAPDIR = ''
DEFIMAP_M,INSTMAP,NIMAP,CHANS,SUFX,CHATTER=0,DIR=IMAPDIR,IGAIN
;! MOVED FROM THE VERY BEGINING OF PROGRAM
;-------------------------------------
;
...
if (nbands eq 0) then begin      ;use input BAND
  band = fix(oparms(6))
  if ((band lt 0) or (band gt nimap)) then begin
  ...
  ind = where( (bands ge 0) and (bands le nimap), ngood )
  if (ngood eq 0) then begin
    print,' Allowed range of values for band = 0 to ',nimap,'.'
...
;     forprint,bands,'   '+instmap(bands+bandadd*nimap-1)
     FORPRINT,BANDS, STRARR(NBANDS)+'   -',BANDS+BANDWS-1 ;!
  endif
endelse  
nbands = n_elements(bands)
;
...
obseq2 = oparms(0)
dir2 = oparms(1)
if (proc eq 'MPE') then ext2 = '_eventrates.tfits' else ext2 = '.evr'
match_files,dir2,obseq2,ext2,name2,nlist2        ;look for the right files
evrfile = name2(0)                               ;name of events rates file
if (proc eq 'MPE') then ext2 = '_attitude.tfits' else ext2 = '.cas'
match_files,dir2,obseq2,ext2,name2,nlist2        ;look for the right files
...
;-----------------------------------------------------------------
; modified on Oct 11, 1993 by wqd
...
;
GET_DETMAP,BANDS,BANDWS,IB,NIMAP,BANDADD,INSTMAP,RMAP,HDMAP,DETMAP $
	,BANDCR=BANDCR ;!
;---------------------------------------------------------------------
...
;                       ext = sufx(ibb-1)+ext
    IF NAME EQ '' THEN NAME=!SEQ_NO ;!
    TAIL='_exp'+STRTRIM(BANDS(IB),2)+STRTRIM(BANDS(IB)+BANDWS(IB)-1,2)+EXPTAIL
    IF (EXT EQ '') THEN EXT = TAIL+'.fits' ELSE $
                        EXT = TAIL+EXT
;-----------------------------------------
...
; delx, dely, and flive.
;
  if (chatter ge 1) then print,' Accumulating exposure map for '
...
;    rmap = rot(map,roll(nn),1.,256.4,256.4)    ;rotate cwise by roll(nn) deg
    rmap = rot(map,roll(nn),1.,256.4,256.4,/inter)    ;rotate cwise by roll(nn) deg
; without the keyword interp, the rot funtion can be erroneous. wqd Jun 8, 1994
;
...
;
      if (chatter ge 2) then print,'   offset ',ii,' of ',noff-1
    endfor      

...
; completely rewriten on May 4, 1994 to make the program simple 
;
;-
;----------------------------------------------------------------------
...
backloc=where(filter(loc) gt 0.,nbackloc) ;regions going to be used
if nbackloc ne 0 then list_b=list(backloc) else $
	list_b=-1
souloc=where(filter(loc) lt 0.,nsouloc) ;regions considered to be sources
if nsouloc ne 0 then list_s=list(souloc) else $
	list_s=-1
numpix_s=nsouloc*(block*block)
numpix_b=(nbackloc)*(block*block)
;
end

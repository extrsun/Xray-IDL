...
,equi1=equi1,equi2=equi2,xnbin=xnbin,ynbin=ynbin,down=down,noloop=noloop
;+
...
; noloop - if set, one-element output 
;
; writen by WQD 4/23/93
;-
if n_params() eq 0 then begin
print,'CALLING SEQUENCE - cursor_posi,corner,hdr,srav,sdecv,cra=cra,cdec=cdec,'
print,'xarcmin=xarcmin,yarcmin=yarcmin,equi1=equi1,equi2=equi2'
print,',xnbin=xnbin,ynbin=ynbin,down=down,noloop=noloop'
return
endif
...
sdecv=[-999d]
LOOP:
cursor,xpix,ypix,1,/device,/up,/change ;,/down
if !ERR eq 4 then goto,done
xp=(xpix-xmid)*xpixpix
yp=(ypix-ymid)*ypixpix
...
if n_elements(xnbin) ne 0 then begin
	...
	ybin=ypix
endelse
print,form="($,4x,2(f7.1,1x),a1,2(2I4,f6.2),2d10.4,a)",xbin,ybin,';',ih,im,is,jd,jm,js,sra,sdec,cr
srav=[srav,sra]
sdecv=[sdecv,sdec]
if not keyword_set(noloop) then goto,LOOP
done:
srav=srav(1:*)
sdecv=sdecv(1:*)
print,''

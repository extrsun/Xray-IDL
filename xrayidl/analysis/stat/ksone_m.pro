 pro ksone_m, data, func_name, d, prob, PLOT = plot, _EXTRA = extra
;+
; NAME:
;	KSONE
; PURPOSE:
...
; 	Modified so that no sort of the data will be done. The input data
;	should be presorted.
;-
 On_error, 2
 ...
 f0 = findgen(N)/ N
; fn = ( findgen( N ) +1. ) / N
 ff = call_function( func_name, data )
...
D = max( [ max( abs(f0-ff), sub0 ), max( abs(fn-ff), subn ) ], msub )
 ...
 if keyword_set(plot) then begin
	...
	plot, data,f0,psym=10,_EXTRA = extra 
	plots, [data(sub0), data(sub0)], [0,1]
     endif else begin
	plot, data,fn,psym=10,_EXTRA = extra
	plots, [data(subn), data(subn)], [0,1]
    endelse 
    oplot,data,ff,lines=1
endif

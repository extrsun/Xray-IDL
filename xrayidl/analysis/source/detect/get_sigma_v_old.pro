...
pro get_sigma_v, count_array, time_array, sigma_array, cntr_array,  $
     radius=radius,core_size=core_size,annulus_in=annulus_in $
	,annulus_out=annulus_out,sfrac=sfrac,bfrac=bfrac

sz=size(count_array)
if n_elements(bin_sel) eq 0 then begin 
	dist_circle,dis,array_size,(sz(1)-1.)/2.,(sz(2)-1.)/2.
...
;
get_snr,core_count,core_time,annulus_count,annulus_time,csn,snr $

...
;-
;++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
pro anal, cra,cdec,array_c,array_t,array_tsub, $
...
if n_elements(outr) eq 0 then outr=60. ;arcmin
...
annulus_out = annulus_out < fix(array_size/2.-dis/bin_arcmin-1-ssr) 
		;cut the outer outannulus to fit into the image
hboxdim=fix(ssr+annulus_out+1)
jmin=(j-hboxdim) 
jmax=(j+hboxdim) 
imin=(i-hboxdim)
imax=(i+hboxdim)
...
for k=0L,(ns-1) do begin
	...
	get_sigma, array_cs,array_ts,sigma_array,cntr_array, $
	core_size=core_size(k),annulus_in=annulus_in(k) $
	,annulus_out=annulus_out(k),radius=ssr(k),sfrac=sfrac,bfrac=bfrac 
;	,snb_array=snb_array

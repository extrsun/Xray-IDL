...
;
;*CALLING SEQUENCE:
; scan_v, center_ra,center_dec, array_c, array_t, array_tsub, block=block,
; radius=radius, blow=blow,bhigh=bhigh, threshold=threshold,iterate, file=file, /append
;
;*PARAMETERS:
;*INPUTS:
...
; add keyword blow and bhigh to be passed to get_sigma_v, WQD, Sept 11, 1993
;-
;++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
pro scan_v, center_ra,center_dec, array_c, array_t, array_tsub, block=block, $
radius=radius, blow=blow,bhigh=bhigh, threshold=threshold, iterate=iterate,file=file $
,append=append,frac=frac,sigma_array=sigma_array
;
if n_params() eq 0 then begin
print,'CALLLING SEQUENCE - scan_v, center_ra,center_dec, array_c, array_t,'
print,' array_tsub, block=block, radius=radius, blow=blow,bhigh=bhigh, threshold=threshold,'
...
; map out the sigma velues in the field
	get_sigma_v, array_c,array_tsub,sigma_array,cntr_array, $
	block=block,radius=radius,blow=blow,bhigh=bhigh,factor=frac
...
;	subtract detected sources from the field
	...
	center_ra,center_dec,star_ra,star_dec,cntr,blow=blow,bhigh=bhigh,block=block,factor=1.5)

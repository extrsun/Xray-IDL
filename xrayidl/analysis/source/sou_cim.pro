pro sou_cim,list,hdr,sra,sdec,asdim,xnp,detagname=detagname,cor=cor,sn=sn,charsize=charsize,thick=thick,xlsh=xlsh,ylsh=ylsh,color=color,shdr=shdr,pcor=pcor,sxmin=sxmin,symin=symin,sdim=sdim
;+
...
;lcrval=sxpar(hdr,'tcrvl*')
;ldel=sxpar(hdr,'tcdlt2')
;lcrpix=sxpar(hdr,'tcrpx*')
lcrval=sxpar(hdr,'crval*')
ldel=sxpar(hdr,'cdelt2')
lcrpix=sxpar(hdr,'crpix*')
sdim=nint(asdim/ldel/3600.)
ns=n_elements(sra)
...
;    print,'Source ',k
    trans_dist,lcrval(0),lcrval(1),sra(k),sdec(k),xp,yp,pix=ldel*3600.,/deg
    ...
    if ns eq 1 then begin
        ...
        get_fitshead,fltarr(sdim(0),sdim(1)),shdr,del=ldel,cpx=lcrpix(0)-cdim(0),cpy=lcrpix(1)-cdim(1),crval=lcrval
    endif

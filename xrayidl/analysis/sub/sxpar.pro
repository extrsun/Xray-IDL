	...
	value = strtrim(strmid(line,8,71),2) else for i = 0,matches-1 do begin
      if ( strmid(svalue(i),0,1) EQ "'" ) then begin   ;Is it a string?
		  ...
		  endap = strpos(test, "'", next_char)      ;Ending apostrophe 
	     	  if endap LT 0 then $
		     endap = strpos(test, "/", next_char) ;Alternative Ending
			;as sql query of ads catalog ; added by wqd, 2/96
		  if endap LT 0 then $
;			    stop,'Value of '+name+' invalid in ' +abort
...
;	       value = strtrim( strmid(line(i), 10, 30), 2)   ;Extract value
; modified by wqd to accormmodate longer strings in ASCA headers, 3/9/95
		aa=line(i)
    	       value = strtrim( strmid(line(i), 10, strlen(gettok(aa,'/'))-10), 2)   ;Extract value    
...
;
if keyword_set(complex) ne 0 then begin ;commend this out, wqd 11/7/96

...
; ,xc=x,yc=y,radiuslow=radiuslow,radiushi=radiushi,plotoff=plotoff
; ,image_b=image_b,ringwidth=ringwidth,rvlo=rvlo,rvhi=rvhi
;
;*PARAMETERS:
; INPUTS:
...
; rvlo,rvhi - lower and upper limits of the dist
; ringwidth - width of each dist bin
; expt,back - exposure and background vectors
; 
;*OPTIONAL OUTPUTS:
...
; add rvlo,rvhi keywords, May 21, 2005
;-
;++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
,image_b=image_b,climit=climit,filter=filter,ee=ee,al=al,expt=expt,back=back,ringwidth=ringwidth,rvlo=rvlo,rvhi=rvhi,rtv=rtv
;---------------------------------------------------------------
if n_params() EQ 0 then begin
print,'CALL SEQUENCE -  rbp,image_c,dist,flux,flux_err,image_t=image_t,block=block'
print,',xc=x,yc=y,radiuslow=radiuslow,radiushi=radiushi,plotoff=plotoff,image_b=image_b,climit=climit,filter=filter,ee=ee,al=al,expt=expt,back=back,ringwidth=ringwidth,rvlo=rvlo,rvhi=rvhi'
retall
endif
;---------------------------------------------------------------
...
if keyword_set(plotoff) eq 0 then begin
    if n_elements(rtv) eq 0 then rtv=rhi < 20
  if rlow ne 0 then tvcircle,rlow,x,y    
  tvcircle,rtv,x,y      ;draws a circle around the source
  tvcircle,rhi,x,y    
endif
;
...
while yn do begin
  ...
  rvlo=(rlow+findgen(rdivs)*ringwidth)/ambin
  rvhi=(rlow+(findgen(rdivs)+1)*ringwidth)/ambin
;
  	count=fltarr(rdivs)
	back=fltarr(rdivs)
  	expt=fltarr(rdivs)
...
;  endif
   if keyword_set(plotoffs) eq 0 then  begin ;and sz(3) ne 4 
  	errplot,dist,flux+flux_err,flux-flux_err
	yn='yes'
	again:  read,'Want to change the radius (yes or no)? ',yn
	yesno,yn
	if yn then begin
		read,'Give the lower and upper  radius limits (arcmin): ',rlow,radiushi
		rhi=radiushi/(!size_pixel*block)
		rlow=radiuslow/(!size_pixel*block)
	endif
   endif else yn=0
endwhile
;

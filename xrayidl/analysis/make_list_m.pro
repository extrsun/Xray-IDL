...
;-
;-------------------------------------------------------------------------------
pro make_list,inputs,plist,plinfo,plhdr,oparms=oparms,tfile=tfile,xytpionly=xytpionly,xshift=xshift,yshift=yshift,ashift=ashift,extyp=extyp
;
...
if n_elements(extyp) eq 0 then extyp = 'fits'                         
case proc of
  'US': ext = '.'+extyp
...
ans = ''
while ( (ctyp ne 'DET') and (ctyp ne 'SKY') and (ctyp ne 'RAW') ) do begin
...
;  addy = -defycen
;endif
; I use mpeshift in the below (wqd)
...
;-------------------
 if proc eq 'MPE' then begin
   if !instr eq 'p' then mpeshift=7680 $ ;shift SKY pixel values to be same as
	else mpeshift=4096
 endif 	else mpeshift=0		;those in US data. added by wqd July,12,93
;-----------------
start = 0 
;num = 20000. 
...
;
   if n_elements(xytpionly) eq 0 then begin ;to save time. wqd July 10, 1992
    case proc of
      'US': ftype = 'dx'
      'MPE': ftype = 'xdet'
      'RDF': if !instr eq 'p' then ftype = 'detx' else ftype='rawx'
    endcase
    dxev= [dxev,  fits_get(hdr,tab,ftype)]
    case proc of
      'US': ftype = 'dy'
      'MPE': ftype = 'ydet'
      'RDF': if !instr eq 'p' then ftype = 'dety' else ftype='rawy'
    endcase
    ...
    case proc of
      'US': ftype = 'pi'
      'MPE': if !instr ne 'h' then ftype = 'ampl' else ftype='raw_ampl'
      'RDF': ftype = 'pi'
    endcase
...
;
    ...
    np = num
    check = 1
    indy = indgen(finish-start+1)  ;add on Feb 17, 1995
;
  ;\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
  if shiftsel ne 0 then begin
	...
	if (ctyp eq 'DET') then begin 
		    case proc of
      			'US': ftype = 'dx'
      			'MPE': ftype = 'xdet'
      			'RDF': if !instr eq 'p' then ftype = 'detx' $
				else ftype='rawx'
    		    endcase
...
;-------------------------------------------
     if(tmin ge 0.) then begin ;replaced by wqd
      	...
      	if kk eq 0 then indt=-1 else begin
		indt=indt(0:kk-1)
      		indt=indt(sort(indt))
      		sel=0
	endelse
...
; esel is added by wqd. Aug 1992
	...
	ftype = 'pi'
    	if (proc eq 'MPE') then begin
		if !instr ne 'h' then ftype = 'ampl' else ftype='raw_ampl'
	endif
 	try = fits_get(hdr,tab,ftype,indy)
      endelse    

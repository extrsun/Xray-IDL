...
; pixels with 0 count or flux do not need to be included  
;else nbin=totnbin ; when the array is real
;
if nbin eq 0  then begin
stop,'No useful bin or count in the image'
return
endif 
;
...
if selv eq 1 then countbm=countm else $
	countbm=total(nimage_c(bin_selb))/float(totnbinb)

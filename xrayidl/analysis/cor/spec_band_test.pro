pro spec_band_test,file,frac,fileout=fileout,rate=rate,sigrate=sigrate,group=group $
,syserr=syserr
;+ 
...
rate=fltarr(8)
sigrate=rate
;norm=1./sqrt(abs(b2(bhigh-blow))) ;surposed to be the channel 7
;
;ratio=sign(b2)*sqrt(abs(b2))*norm
;ratio_err=ratio*(b2err*0.5)/sqrt(abs(b2)*abs(b2))
bl=sqrt(b2 - b2err > 0.00001)
bh=sqrt(b2 + b2err > 0.00001)
b=(bl+bh)*0.5
berr=(bh-bl)*0.5
ratio=b/b(bhigh-blow)
ratio_err=berr/b(bhigh-blow)
...
sigrate(blow-1:bhigh-1)=abs(rate(blow-1:bhigh-1))* $
	sqrt(berr^2/b^2 + 1./counts + syserr*syserr)
norm=frac/rate(bhigh-1)
rate=norm*rate
sigrate=norm*sigrate

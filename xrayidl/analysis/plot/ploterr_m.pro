...
PRO ploterr, x, y, xerr, yerr, NOHAT=hat, HATLENGTH=hln, ERRTHICK=eth, $
      ERRSTYLE=est, TYPE=type, BACKGROUND=back, CHANNEL=chan, CHARSIZE=chsiz, $
...
;
; CALLING SEQUENCE:
;	ploterr, [ x,]  y, [xerr], yerr [, TYPE = type] [,/NOHAT, HATLENGTH=hln,
...
if n_elements(yrange) eq 0 then yrange=minmax(y)

 pro imlabelmmm,hdr,pos=pos,xtitle=xtitle,ytitle=ytitle,chsize=chsize, $
  ...
  xunitlv=xunitlv,yunitlv=yunitlv,toplabel=toplabel,botlabel=botlabel,leftlabel=leftlabel,ticklen=ticklen
;+
; NAME:
;       IMLABELMMM
;
...
if n_elements(yunitlv) eq 0 then yunitlv = .10
...
;** of the image:
  ...
  if pole eq 0 then begin
    ...
    minra = min([botmin,botmax,topmin,ra0]) ;-1.e-4 ;for the calculation accuracy
    maxra = max([botmin,botmax,topmin,ra0]) ;+1.e-4
    rasize = maxra - minra

    mindec = min([leftmin,leftmax,rightmin,dec0,dec1]) ;-1.e-4
    maxdec = max([leftmin,leftmax,rightmin,dec0,dec1]) ;+1.e-4
...
;     xdis = nx1 + xticklen*xval/(xsize-1) & ydis = ny1+yticklen*yval/(ysize-1)
...
;            xyouts, nx1*.95, yt1, label, /normal, size=chsize*0.9, color=f_color, alignment=1.0
             n = 1
         endif else n = 0
 	endif
...
;     if botlabel eq 1 then begin
       ...
       if min(ydis) lt ny1 then begin
         ...
         if nxsel ne 0 then begin
           ...
           ;** Label tick marks (every o other tick):
...
;     if toplabel eq 1 then begin
       ...
       if max(ydis)-ny2 gt 0 then begin 
;       if max(ydis)-ny2 gt -1.e-4 then begin 
		;1.e-6 is for the accuracy of the calculation
...
;   xyouts, nx1+.5*xticklen, xunitlv*ny1, xunits, size=chsize*1.00, $
 ...
 if toplabel eq 1 then $
   xyouts, nx1+.5*xticklen, ny2+xunitlv*(ny2-ny1), xunits, size=chsize*1.00, $
     color=f_color, alignment=0.5, /normal

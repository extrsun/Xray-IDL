pro ticlabels_m, minval, numtics, incr, ticlabs, RA=ra, DELTA = delta
;+
; NAME:
;	TICLABELS
; PURPOSE:
...
;                               convert min to hours, minutes, secs
  ...
  maxval=firstval+ numtics*numtics/60.
...
if maxval lt 10 then begin
      ticlabs(0) = sgn + string( abs(minh), '(i2.1)') + sd + ' ' + $
               string(minm,'(i2.2)') + sm + ' ' + string( fix(mins), '(i2)') + ss
endif else begin
      ticlabs(0) = sgn + string( abs(minh), '(i2.2)') + sd + ' ' + $
               string(minm,'(i2.2)') + sm + ' ' + string( fix(mins), '(i2)') + ss

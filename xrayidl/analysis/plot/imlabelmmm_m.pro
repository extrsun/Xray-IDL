pro imlabelmmm,hdr,pos=pos,xtitle=xtitle,ytitle=ytitle,chsize=chsize, $
  ...
  xunitlv=xunitlv,yunitlv=yunitlv,toplabel=toplabel,botlabel=botlabel,leftlabel=leftlabel,tick2=tick2
;+
; NAME:
;       IMLABELMMM
;
...
if n_elements(xunitlv) eq 0 then xunitlv = .1
if n_elements(yunitlv) eq 0 then yunitlv = .1
...
;     constdec = replicate(leftmin2+decdir*decincr*fix(m)/60., plint+1)
	...
	if leftlabel ne  0 then begin
	 ...
	 if (m/2. eq float(m/2)) then ddx=dx/50. else ddx=dx/100.
         plots, [nx1,nx1+ddx], [yt1,yt1], /normal, color=f_color
	 if n_elements(tick2) ne 0 then $
		plots, [nx2,nx2-ddx], [yt1,yt1], /normal, color=f_color
     endif
...
;     constra = replicate(botmin2+radir*raincr*fix(m)*360./24./60., plint+1)
	 ...
	 if (m/2. eq float(m/2)) then ddx=dx/50. else ddx=dx/100.
         plots, [xt1,xt1], [ny1,ny1+ddx], /normal, color=f_color
	 if n_elements(tick2) ne 0 then $
		plots, [xt1,xt1], [ny2,ny2-ddx], /normal, color=f_color
        endif
     endif
...
;   xyouts, nx1+.5*dx, xunitlv*ny1, xunits, size=chsize*1.00, $
   xyouts, nx1+.5*dx,ny1-xunitlv*(ny2-ny1), xunits, size=chsize*1.00, $
     color=f_color, alignment=0.5, /normal
 if botlabel eq 0 and toplabel eq 1 then $
   xyouts, nx1+.5*dx, ny2+xunitlv*(ny2-ny1), xunits, size=chsize*1.00, $
     color=f_color, alignment=0.5, /normal

 ;** Place y label:

if leftlabel ne 0 then $
 xyouts, nx1-yunitlv*(nx2-nx1), ny1+.5*dy, yunits, size=chsize*1.00, $

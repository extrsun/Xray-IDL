...
pro sh_plot,x,flux,eflux,mflux,title=title,xfactor=xfactor,yfactor=yfactor, $
 errl=errl,erru=erru,xrange=xrange,yrange=yrange,xytitle=xytitle,mflux2=mflux2 $
,charsize=charsize $
	,ytitle=ytitle,xtitle=xtitle,flo=flo,fhi=fhi,pthick=pthick,psym=psym
...
if n_params() eq 0 then begin
  print,'CALLING SEQUENCE - sh_plot,x,flux,eflux,mflux,title=title,'
  print,'  xfactor=xfactor,yfactor=yfactor,errl=errl,erru=erru'
  print,',xrange=xrange,xytitle=xytitle,mflux2=mflux2,ytitle=ytitle,xtitle=xtitle,flo=flo,fhi=fhi,psym=psym,charsize=charsize'
  return
endif
...
if n_elements(pthick) eq 0 then !p.thick=2. else !p.thick=pthick
if n_elements(charsize) eq 0 then begin
    if !p.multi(2) eq 2 then !p.charsize= 1.5 else !p.charsize=1.3
...
if n_elements(xrange) ne 0 then begin
  if n_elements(xytitle) ne 0 then begin
    plot,x*xfactor,flux*yfactor,xrange=xplrange,xstyle=1,ystyle=1, $
      yrange=yrange, $
      xtitle=xtitle,ytitle=ytitle,xthick=2.,ythick=2.,zthick=2.,psym=1
;    if n_elements(mflux2) ne 0 then oplot,x*xfactor,mflux2*yfactor,linestyle=2
    ...
    plot,x*xfactor,flux*yfactor,xrange=xplrange,xstyle=1,ystyle=1, $
      yrange=yrange, $
      title='!6'+title,xtitle=xtitle,ytitle=ytitle,xthick=2.,ythick=2.,zthick=2.,psym=1
;    if n_elements(mflux2) ne 0 then oplot,x*xfactor,mflux2*yfactor,linestyle=2
  endelse
endif else begin
  if n_elements(xytitle) ne 0 then begin
    plot,x*xfactor,flux*yfactor,xrange=xplrange, $
      yrange=yrange, $
      xtitle=xtitle,ytitle=ytitle,xthick=2.,ythick=2.,zthick=2.,xstyle=1,ystyle=1.,psym=1
;    if n_elements(mflux2) ne 0 then oplot,x*xfactor,mflux2*yfactor,linestyle=2
    xyouts,xytitle(0),xytitle(1),title,charsize=charsize,charthick=2.
  endif else begin
    plot,x*xfactor,flux*yfactor,xrange=xplrange,xstyle=1,ystyle=1, $
...
if n_elements(errl) ne 0 and n_elements(erru) ne 0 then begin
    ss=where(errl ge xplrange(0) and erru le xplrange(1),nss)
    if nss ne 0 then errplot_y,flux(ss)*yfactor,errl(ss),erru(ss)
endif

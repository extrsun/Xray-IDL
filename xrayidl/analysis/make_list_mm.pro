...
;if !debug eq 1 then stop
	...
	ftype='x'
      ...
      endelse
;if min(ytry) le 9000 then if !debug eq 1 then stop
      ...
      if (ymin eq 0) then ybot = min(ytry)
       ...
       if (xmin eq 0) then xbot = min(ytry)
       if (xmax eq 0) then xtop = max(ytry)

      indy = where( (ytry ge ybot) and (ytry le ytop) and $
		(xtry ge xbot) and (xtry le xtop) )

if min(ytry) le 9000 then if !debug eq 1 then stop,'stop 2'

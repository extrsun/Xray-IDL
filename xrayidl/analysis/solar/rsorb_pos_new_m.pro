...
;
;*CALLING SEQUENCE:
;       rsorb_pos, inputs, obinum, time, satpos, orbvecs, relpos,
...
;    mode 1) Interactive
;        IDL>rsorb_pos,1,obinum,time,satpos, ...
;             ?obseq=rp123456
;	      ?dir=mydirectory
;             ?instr=P
;	      ?chatter=0
...
;        IDL>rsorb_pos,list,obinum,time,satpos, ...
;  
...
;	     chatter=0
;            *exit
;        IDL>rsorb_pos,'myinput.dat',obinum,time,satpos, ...
;
;*RESTRICTIONS:
;
;*NOTES:
...
;    apparent problem with the output time (it does not monotonically increase)
;    It is now changed to UT time and the procedure is renamed. Nov 16 (wqd) 
;-
;-------------------------------------------------------------------------------
pro rsorb_pos,inputs,obinum,time,satpos,orbvecs,relpos,obinfo=obinfo,oparms=oparms
;
npar = n_params(0)
if (npar eq 0) then begin
  print,' RSORB_POS, inputs, obinum (0 for all), TIME, SATPOS, ORBVECS, '
...
;
  ...
  entry = tbget(hdr,tab,ftype)/1.e4       ;subsecond time (in sec)
  nument = n_elements(entry)                 ;number in this section
  ftype = 'IUT2_SO'
  if (instr eq 'H') then ftype = 'UNIVERSAL TIME - SECONDS'
  entry = entry + tbget(hdr,tab,ftype)         ;time in seconds
;  time = [time,entry] ;entry includes the time of the day 
; if OBIs were taken on different days, entry does not monitonically increase
; commented by wqd
;
...
;
  ...
  entry = tbget(hdr,tab,ftype)                 ;date in YYMMDD format
  begdate = entry(0)
  enddate = entry(nument-1)
  rsobitimes,hdr,sctb,scte,utb,ute          ;get OBI start and end times 
;
...
;
  ...
  TIME=[TIME,SCTB+ENTRY_M-ENTRY_M(0)] ;ABOLUTE SPACE CRAFT TIME
  sctbeg(jj) = sctb
  yy = fix(begdate/1.e4)  
...
;
  if (npar ge 4) then begin
    ...
    entry = tbget(hdr,tab,ftype)/10./3600.     ;longitude in decimal degrees
    rs_lon = [rs_lon,entry]
;
    ftype = 'ILAT_SO'
    if (instr eq 'H') then ftype = 'GEODETIC LATITUDE NORTH'
    entry =tbget(hdr,tab,ftype)/10./3600.      ;latitude in decimal degrees
    rs_lat = [rs_lat,entry]
;
    ftype = 'IALT_SO'
    if (instr eq 'H') then ftype = 'ALTITUDE'
    entry = tbget(hdr,tab,ftype)/1000.         ;satellite altitude in kilometers
    height = [height,entry]
  endif
;
  if (npar ge 5) then begin
    ...
    entry = tbget(hdr,tab,ftype)/1.e8          ;x comp Earth-Sun unit vector
    sunx = [sunx,entry]
;
    ftype = 'ISUN_SOY'
    if (instr eq 'H') then ftype = 'SUN UNIT VECTOR - Y COMPONENT'
    entry = tbget(hdr,tab,ftype)/1.e8          ;y comp
    suny = [suny,entry]
;
    ftype = 'ISUN_SOZ'
    if (instr eq 'H') then ftype = 'SUN UNIT VECTOR - Z COMPONENT'
    entry = tbget(hdr,tab,ftype)/1.e8          ;z comp
    sunz  = [sunz ,entry]
;
    ftype = 'MOON_SOX'
    if (instr eq 'H') then ftype = 'MOON UNIT VECTOR - X COMPONENT'
    entry = tbget(hdr,tab,ftype)/1.e8          ;x comp Earth-Moon unit vector
    moonx = [moonx,entry]
;
    ftype = 'MOON_SOY'
...
;
    ...
    entry = tbget(hdr,tab,ftype)/1.e8        ;x comp Earth-Satellite unit vector
    satx = [satx,entry]
;
    ftype = 'ISAT_SOY'
...
;
  if (npar ge 6) then begin
    ...
    entry = tbget(hdr,tab,ftype)/10./3600.     ;satellite azimuth in dec deg
    sataz = [sataz,entry]
;
    ftype = 'IELS_SO'
    if (instr eq 'H') then ftype = 'ELEVATION'
    entry = tbget(hdr,tab,ftype)/10./3600.     ;satellite elevation in dec deg
    satel = [satel,entry]
;
    ftype = 'IGHA_SO'
    if (instr eq 'H') then ftype = 'GREENWICH HOUR ANGLE'
    entry = tbget(hdr,tab,ftype)/10./3600.     ;Greenwich Hour Angle in dec deg
    satgha = [satgha,entry]
;
    ftype = 'IDIS_SO'
    if (instr eq 'H') then ftype = 'WEILHEIM STATION DISTANCE'
    entry = tbget(hdr,tab,ftype)/1000.         ;distance from station in kilom
...
if (!debug gt 0) then stop,'Stopping in rsorb_pos after iend defined'
;
row={obitime,sctbeg:0.0D0,yrbeg:0,daybeg:0,utsbeg:0.0D0,$

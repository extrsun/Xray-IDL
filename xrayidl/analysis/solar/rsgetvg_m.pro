...
; problem with the input time from rsorb_pos is not yet fixed. as a temporary
; solution, another parameter scttime is included to calculate the correct
; space craft time of the data points. Nov 16, 1992, (WQD)
;-
;-------------------------------------------------------------------------------
pro rsgetvg,inputs,obinum,time,vginfo,orbvecs,obinfo=obinfo,oparms=oparms,SCTTIME=SCTTIME
;
npar = n_params(0)
if (npar eq 0) then begin
  print,' RSGETVG, inputs, obinum (0 for all), TIME, VGINFO, ORBVECS, '
  print,'       OBINFO=OBINFO, OPARMS=OPARMS,SCTTIME=SCTTIME'
...
sxhread,hdrfil,hdr
getnomasp,hdr,nomra,nomdec,nomroll
...
ind = where( sesang lt seslim(2) ) & help,ind
...
;
ind = where(sscode eq 0)
if (ind(0) ge 0) then vgcalc(ind) = 1
ind = where((epcode eq 0) and (sscode gt 0))
if (ind(0) ge 0) then vgcalc(ind) = 2
ind = where((epcode gt 0) and (sscode eq 1))
if (ind(0) ge 0) then vgcalc(ind) = 3
ind = where((epcode eq 1) and (sscode eq 2))
if (ind(0) ge 0) then vgcalc(ind) = 3
ind = where((epcode gt 1) and (sscode eq 2))
if (ind(0) ge 0) then vgcalc(ind) = 4
ind = where((epcode eq 3))

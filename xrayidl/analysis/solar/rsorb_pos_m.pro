...
;-
;-------------------------------------------------------------------------------
pro rsorb_pos,inputs,obinum,time,satpos,orbvecs,relpos,obinfo=obinfo,oparms=oparms,SCTTIME=SCTTIME
;
npar = n_params(0)
if (npar eq 0) then begin
  ...
  print,'   Uses inputs OBSEQ, DIR, EXTYP, INSTR, PROC, and CHATTER ' $
       +'from RSGET.DEF,SCTTIME=SCTTIME'
  retall
endif
;
dfile = 'rsget'
...
;  if (!debug gt 1) then print,nument,minmax(entry),entry(0),entry(nument-1)
;  ind = where( (entry ge (utb(2)-100.)) and (entry le (ute(2)+100.)) )   
;  entry = entry(ind)
  nument = n_elements(entry)                  ;number in this section
  if (!debug gt 1) then print,nument,minmax(entry),entry(0),entry(nument-1)
...
;
  ...
  SCTTIME=[SCTTIME,SCTB+TIME_OB-TIME_OB(0)] ;ABOLUTE SPACE CRAFT TIME
  sctend(jj) = scte
  yy = fix(enddate/1.e4)  

...
,binrat=binrat,offaxis=offaxis,hpsfdim=hpsfdim,clim=clim,imonly=imonly,cstat=cstat,so=so,psffto=psffto
;+
...
if n_params() lt 1 then begin
 ...
 print,',offaxis=offaxis,hpsfdim=hpsfdim,psffto=psffto (PSF parameters)'
 print,',imonly=imonly,cstat=cstat,so=so'
 return
endif

common powell_cl,locs,choice,modelc,imagec,psfim,psfft
	;common block to be shared by func_powell
forward_function func_powell
...
;---------------------------------
; get the psf
if n_elements(psffto) ne 0 then begin
	sz=size(psffto)
	hpsfdim=sz(1)/2 ;assuming square for psffto
	psfft=psffto
endif else begin 
...
imagec=[dim,binrat,block,hpsfdim]
...
if keyword_set(imonly) eq 0 then mle_cl,para,choice=choice,verb=verb
cstat=func_powell(para)
print,'cstat, para = ',cstat,para

cl_imc,dim,para,clim,binrat=binrat,psfim=psfim,psfft=psfft $
...
if n_elements(psffto) eq 0 then psffto=psfft ;for conf_bts_cl.pro
return	
end
;======================================
...
for k=0,npara-1 do xi(k,k)=para(k)*0.5
;for k=0,npara-1 do xi(k,k)=para(k)

nr_powell,para,xi,1.e-4,fmin,'func_powell',itmax=400,iter=iter
...
cl_imc,imagec(0),xx,clim,binrat=imagec(1),psfim=psfim,psfft=psfft $
 ,hpsfdim=imagec(n_elements(imagec)-1),block=imagec(2)
xx=abs(xx) ;keeping them positive
case choice of 
 1: 	begin 
...
;======================================
pro cl_imc,dim,para,clim,binrat=binrat,psfim=psfim,psfft=psfft $
	,hpsfdim=hpsfdim,block=block
;+
...
if n_params() eq 0 then begin 
 ...
 print,',hpsfdim=hpsfdim,block=block'
 return
endif
if n_elements(binrat) eq 0 then binrat=1
 calcl,dim+2*hpsfdim,(block*!size_pixel)/para(1),clim,binrat=binrat,modelp=para,chatt=0

...
;	fix the bugs when the number of OBi greater than 9 using gettok_m
;	Nov 17, 1992, (WQD)
;-
;-------------------------------------------------------------------------------
...
if (npar ge 5) then begin                  ;look for sequence start times
  ...
  seqbeg(0) = value
;
  gethistval,hdr,'Sequence start time UTC (YYMMDD) =',utyr,lnum,tok=token,ch=0
  gethistval,hdr,'Sequence start within YYMMDD UTC (millisec) =',utsec,lnum,tok=token,ch=0
  utsec = utsec/1000.
;
  yy = fix(utyr/1.e4)  
...
if (npar ge 6) then begin                  ;look for sequence stop times
  ...
  seqend(0) = value
;
  gethistval,hdr,'Sequence end time UTC (YYMMDD) =',utyr,lnum,tok=token,ch=0
  gethistval,hdr,'Sequence end within YYMMDD UTC (millisec) =',utsec,lnum,tok=token,ch=0
  utsec = utsec/1000.
;
  yy = fix(utyr/1.e4)  
  mm = fix( (utyr-yy*1.e4)/1.e2)
...
;
    tok = gettok_m(hdrlin,strtrim(string(jj+1),2))  ;sct OBI jj+1 start, end time

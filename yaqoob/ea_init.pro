pro ea_init, obsname, det, hpcut, gradeflt, xybin=xybin, echo=echo, $
	recover_time, date=date, outevtfil=outevtfil, loglun=loglun, $
	gisclean=gisclean, aux=aux

if n_params(0) eq 0 then begin
    print,'ea_init, obsname, det, hpcut, gradeflt, xybin=xybin, echo=echo, $'
    print,'    recover_time, date=date, outevtfil=outevtfil, loglun=loglun $'
    print,'    gisclean=gisclean, aux=aux' 
...
if count gt 0 then begin
  ...
  if keyword_set(echo) then begin
    print, sname + ' already exists, delete it if you want the unf'
...
if count eq 0 then begin
  if keyword_set(echo) then begin 
    print, 'No mkf files found in ' + aux + $
      ', please install them to do minimal mkf cleaning'
    print, 'Returning from ea_init.pro'
  endif
  printf, loglun, 'No mkf files found in ' + aux + $
    ', please install them to do minimal mkf cleaning'
  printf, loglun, 'Returning from ea_init.pro'
  return
endif
ea_gunzip, flist

unflist = ea_fingun("*" + short_det_name(det) + "*unf*")

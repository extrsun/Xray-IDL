pro qmkpha,cnts,fname=fn,tm=tm,npx=npx,back=back,resp=resp,cor=cor
;+
;NAME:
;mkpha
;
;PURPOSE:
;to convert spectral counts into XSPEC format
;
;CALLING SEQUENCE:
;INPUTS:
...
print,'QMKPHA,cnts,fname=fname,tm=tm,npx=npx,back=back,resp=resp,cor=cor
retall
end
...
if n_elements(resp) eq 0 then resp='/home1/pjs/rosat/rosspec/rospspc.rsp'
...
nbin=long(n_elements(cnts)) & nchan=256L
A=1.0
cor=0.0
indices=fltarr(3)*0.
spare=intarr(48)*0

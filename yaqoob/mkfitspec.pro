pro mkfitspec,plist,geo,gti,texp,x0,y0,wbin,wmap,npha,evtfile,sname=sname,styp=styp,bck=bck,rsp=rsp,arf=arf,ibgd=ibgd
;Author T. Yaqoob - Oct 1995
if n_params(0) eq 0 then begin
 print,'mkfitspec,plist,geo,gti,texp,x0,y0,wbin,wmap,npha,evtfile,sname=sname,styp=styp,bck=bck,rsp=rsp,arf=arf,ibgd=ibgd'
...
if n_elements(styp) eq 0 then begin
 styp=0
inv: print,'Must Specify spectrum type:'
 print,'0: GIS spectrum '
 print,'1: SIS BRIGHT or converted FAINT '
 print,'2: SIS FAINT mode '
 print,'3: PSPC spectrum '
 read,'Enter spectrum type ',styp
 if styp lt 0 and styp gt 3 then begin
  print,'Invalid Response '
  goto,inv
 endif
endif
if styp le 2 then mission='ASCA'
if styp eq 3 then mission='ROSAT'
if geo.inst eq 0 then instr='SIS0'
if geo.inst eq 1 then instr='SIS1'
if geo.inst eq 2 then instr='GIS2'
if geo.inst eq 3 then instr='GIS3'
if geo.inst eq 4 then instr='PSPC'
...
if npha lt 0 then pulse=plist.pha else pulse=plist.pi
if n_elements(ibgd) eq 0 then ibgd=0 
nchan=abs(npha)+0l
sbsiz=1024l/nchan
if styp eq 0 then $
 spec=histogram([0,pulse],max=1023,binsize=sbsiz)
if styp eq 1 then $
 sisbr,pulse,spec,bsz=sbsiz
if styp eq 2 then $
 spec=histogram([0,pulse],max=4095,binsize=4*sbsiz)
if styp eq 3 then $
 spec=histogram([0,pulse],max=255,binsize=(sbsiz/4))
...
mkspechdr,geo,plist,gti,texp,mission,instr,ibgd,npha,x0,y0,wbin,wmap,$

pro qmake_sis_reg,plist,cx,cy,rootname,bin,srcrad,bkgrad,sis=sis,srcregfile=srcregfile,bgdregfile=bgdregfile,ccdopt=ccdopt

	if n_params(0) eq 0 then begin
	  print,'qmake_sis_reg,plist,cx,cy,rootname,bin,srcrad,bkgrad,sis=sis,srcregfile=srcregfile,bgdregfile=bgdregfile,ccdopt=ccdopt'
	  ...
	  print,'If ccdopt eq 0 then use only main chip for bgd else '
	  print,'use all active chips'
	  retall
	endif
;        print,'ON ENTRY NELEMENTS (bkgrad) = ',n_elements(bkgrad) 
;	print,'ON ENTRY bkgrad = ',bkgrad
; This array remaps ccdid to system where 1st bit = column, 2nd bit = row
	remap = [1b,0b,2b,3b]

	xsize = 425
	ysize = 450
	if n_elements(ccdopt) eq 0 then ccdopt=0
...
; based on minmax of main ccd
	...
	a = remap(ccd)
	for i=0,3 do begin
;if ccdopt = 0 then only use the main chip for source region
           if ccdopt gt 0 then begin
	    w=where(plist.ccd eq i,n)
	   endif else begin
	    w = where((plist.ccd eq i) and (mainchip eq i),n)
           endelse
	  x = avg(xr) & y = avg(yr)
...
;if ccdopt=0 then use only the main chip for bgd else use all chips
;which are active
           if ccdopt gt 0 then begin
	    w = where(plist.ccd eq ccd,n)
           endif else begin
	    w = where((plist.ccd eq ccd) and (mainchip eq ccd),n)
	   endelse
           print,'ccd n main',ccd,n,mainchip
	    if n gt 0 then begin
          ...
          print,'bkgrad = ',bkgrad*37.7082/bin
	  printf,1,'-CIRCLE(',strn(bx)+','+strn(by)+','+strn(bkgrad*37.7082/$
	    bin)+')'
	  for i=1,n_elements(mask)-1 do printf,1,mask(i)
	  close,1
	endif else begin
	  ...
	  printf,1,' CIRCLE(',strn(bx)+','+strn(by)+','+strn(bkgrad(1)*37.7082/$
            bin)+')'
	  printf,1,'-CIRCLE(',strn(bx)+','+strn(by)+','+strn(bkgrad(0)*37.7082/$
            bin)+')'
	  for i=1,n_elements(mask)-1 do printf,1,mask(i)

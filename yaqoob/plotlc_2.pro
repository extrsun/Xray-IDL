PRO plotlc_2,x,y,dx,dy,num=num,NOHAT=hat, HATLENGTH=hln, ERRTHICK=eth, $
...
; CALLING SEQUENCE:
;	plotlc_2,x,y,dx,dy,num=num [, TYPE = type] [,/NOHAT, HATLENGTH=hln,
;		 ERRTHICK=eth, ERRSTYLE=est]
; INPUTS:
;	X,Y,DX,DY => data and errorrs
...
print,'PLOTLC_2,x,y,dx,dy,num=num'
...
IF yrange(0) GT yrange(1) THEN BEGIN
	...
	yhi = yy - yerr
ENDIF
ylo=ylo > yrange(0) & yhi=yhi < yrange(1)
;       
...
oldxr=xrange
xlo=xlo > xrange(0) & xhi=xhi < xrange(1)
;
;			Plot the positions.
;
tb2:if num eq 1 then plot,xx,yy,$
...
if aa eq 'q' then goto,tb10
endrep until aa eq 'c'
if fix(total(xrange ne oldxr)/2.) then begin
xrange=oldxr & goto,tb2
endif
ans=' ' & read,'Want to expand a segment? y/(n): ',ans
if ans ne 'y' then goto,tb10
print,'Use cursor to mark ''xmin'' and ''xmax'' '
cursor,xc1,yc,3 & cursor,xc2,yc,3
xrange=[xc1,xc2] & print,xrange & goto,tb2
tb10:RETURN
END

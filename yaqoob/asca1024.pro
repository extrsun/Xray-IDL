pro asca1024,cnts,detid=detid,fname=fn,tm=tm,npx=npx,back=back,resp=resp,cor=cor
...
;
;PURPOSE:
;to put spectral data into XSPEC format using 1024 channels
;
;CALLING SEQUENCE:
;INPUTS:
...
print,'ASCA1024,cnts,detid=detid,fname=fname,tm=tm,npx=npx,back=back,resp=resp,cor=cor
retall
end
...
nbin=long(n_elements(cnts)) & nchan=1024L
A=1.0
cor=0.0
cnts=float(cnts)
indices=fltarr(3)*0.

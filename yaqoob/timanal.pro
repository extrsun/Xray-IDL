...
if n_elements(qdpname) eq 0 then wresults = 0 else wresults=1
;print,'qdpname el ',n_elements(qdpname)
;print,'wresults ',wresults
if wresults gt 0 then openw,1,qdpname
if wresults gt 0 then openw,2,'frac'+qdpname
dummy=0.0
if wresults gt 0 then begin
...
for j=0l,ncurves-1l do begin
 binsize=binwidths(j)
 thalf=halfbins(j) 
 qkcurve,time,gti,binsize,nbins,tcen,tbounds,ctspsec,ctserr,wnz,nnz
...
;	if wresults gt 0 then printf,1,'! mean count rate ',mu
...
;  if tmpcts(i) gt 0. and wresults gt 0 then printf,1,tcen(i),thalf,tmpcts(i),tmpctserr(i)

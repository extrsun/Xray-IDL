...
alpha_inner = -5.18d*!dtor       ; HEG clocking angle
alpha_outer = 4.75d0*!dtor        ; MEG clocking angle, radians
...
; dispersion is:
   w = where((float(y)-y0)^2+(float(z)-z0)^2 gt float(tg_d)^2, n)
   if n eq 0 then stop
   tg_r = tg_d*0.
   tg_r(w) = sqrt((float(y(w))-y0)^2+(float(z(w))-z0)^2 - float(tg_d(w))^2)
